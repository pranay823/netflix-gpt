...
const Header = ( ) =>{
  ...
  return (
    ...
    <div className={`bg-black p-6 md:p-0 md:bg-none   absolute   w-full md:w-auto right-0   md:mr-24 md:h-12 h-48  mt-5 flex  md:flex-row flex-col transition-all ease-in duration-500 md:opacity-100  ${open ? 'opacity-100':'top-[-1000px] opacity-0'}  `}>
     {showGptSearch && <select className="mb-2 md:mb-0 md:mr-2 p-2  " onChange={languageHandler}>
      ...
      className="text-black p-2  bg-red-600 md:mr-2">GPTSearch</button>

      <button onClick={signoutHandler} className=" bg-red-600 p-2 md:mt-0 mt-2  " >Sign out</button>
      </div> </div>)}
    </div>
  );
};
